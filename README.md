...
A clean, modern, and fully responsive Todo / Task Card component built using pure HTML, CSS, and Vanilla JavaScript. This project serves as a foundational exercise focusing strictly on semantic HTML, web accessibility (a11y), responsive design, DOM testability, and imperative UI synchronization.
...
* **Imperative DOM Synchronization:** Shifted to a clean event-driven pattern where the HTML DOM acts as the single source of truth. UI states are synchronized directly via JavaScript without relying on abstract state objects.
* **Edit Mode:** Users can toggle an inline edit form, alter properties, and choose to save or cancel (which reliably restores previous data directly from the view elements).
...
* **Priority Indicator:** Added a persistent visual left-border accent (`.priority-accent`) that maps to the priority data attribute (Red = High, Yellow = Medium, Green = Low).
...
* **Focus Management (Trap/Restore):** Clicking 'Edit' shifts focus to the first input field (`edit-title`). Clicking 'Save' or 'Cancel' returns focus back to the Edit button to maintain smooth, predictable keyboard flow.
...
* Modifications made in Edit Mode or via toggles are not persistent. Reloading the page will reset the card to the hardcoded HTML values, as data persistence (`localStorage` or database) is not implemented in this stage.

## 🛠️ Technologies Used

* **HTML5** (Semantic structure, aria attributes, datetime properties)
* **CSS3** (Flexbox, CSS Variables, Responsive design, Data-attribute styling)
* **Vanilla JavaScript** (Imperative DOM manipulation, Date math, Event delegation)
...
├── script.js    # Interactivity, time calculation, and DOM syncing
