document.addEventListener("DOMContentLoaded", () => {
  ...
  const editDesc = document.querySelector(
  ...
  function updateTimeUI() {
    ...
    if (diff < 0) {
      ...
      timeRemainingEl.textContent =
        hours > 0 ? `Overdue by ${hours}h` : `Overdue by ${mins}m`;
    } else {
      card.classList.remove("is-overdue");
      overdueIndicator.classList.add("hidden");
      ...
      if (days > 0)
        ...
        timeRemainingEl.textContent = `Due in ${hours} hour${hours > 1 ? "s" : ""}`;
      else
        timeRemainingEl.textContent = `Due in ${mins} min${mins > 1 ? "s" : ""}`;
    }
  }
